...
#+BEGIN_SRC jupyter-python  :session /docker:root@tutorial_bastion:/root/.local/share/jupyter/runtime/kernel-ba3d7896-a2bc-4982-83af-f5a056b38a69.json  :results none
%matplotlib inline
import pandas as pd
import numpy as np
...
#+BEGIN_SRC jupyter-python  :session /docker:root@tutorial_bastion:/root/.local/share/jupyter/runtime/kernel-ba3d7896-a2bc-4982-83af-f5a056b38a69.json
...
#+RESULTS:
[[file:./.ob-jupyter/736eb2949da7b589947cce9415e676fa0ec43982.png]]

Every model selected by audition has a very similar performance across
time, and they are ~2.5 times above the baseline in precision@10%.
...
#+RESULTS:
[[file:./.ob-jupyter/b6e15ea8c1e077d0d70464257c39e2c631e0b89a.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/79b5dbc13b72daa1367305d11ca292ff94d0a0fa.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/cc3b2558b6ba321d78baa57913c23332017a0b2b.png]]
...
choose any and it won’t matter. Let’s collect more evidence of this.

Besides the overlap in the list, we can see if the /ranking/ is
correlated. This means that the model groups share not only the
facilities but also the relative rank risk.

#+BEGIN_SRC  jupyter-python  :session /docker:root@tutorial_bastion:/root/.local/share/jupyter/runtime/kernel-ba3d7896-a2bc-4982-83af-f5a056b38a69.json
audited_models_class.plot_ranked_correlation_preds(param_type='rank_pct',
                                                   param=10,
                                                   temporal_comparison=True);
#+END_SRC

#+RESULTS:
[[file:./.ob-jupyter/343964d2e40d09e4fd674d68ad176f928c9704c6.png]]
...
data, let’s see what the particular models are doing
...
#+RESULTS:
[[file:./.ob-jupyter/27f4d846e0b3d96f8e2ce0b61ee086e4232e9aff.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/0704bc064af73877081e0bce264f462a4f8e06ed.png]]


*** Our /Policy menu/

#+BEGIN_SRC jupyter-python :session /docker:root@tutorial_bastion:/root/.local/share/jupyter/runtime/kernel-ba3d7896-a2bc-4982-83af-f5a056b38a69.json
models_64['226'].plot_precision_recall_n(figsize=(8,8))
#+END_SRC

#+RESULTS:
:RESULTS:
: <matplotlib.figure.Figure at 0x7f043a3aa438>
[[file:./.ob-jupyter/0c81e406fc3028ea51bfdf86746ec2bc09d850a2.png]]
:END:
...
#+BEGIN_SRC jupyter-python :session /docker:root@tutorial_bastion:/root/.local/share/jupyter/runtime/kernel-ba3d7896-a2bc-4982-83af-f5a056b38a69.json
models_64['226'].error_analysis(params.thresholds,
...
<ipython-input-25-42ffac78728d> in <module>
      2                       depth=params.max_depth_error_tree,
      3                       path=params.project_path,
----> 4                       view_plots=True)

/usr/local/lib/python3.6/site-packages/triage/component/postmodeling/contrast/model_evaluator.py in error_analysis(self, threshold, **kwargs)
   1091                    print(threshold_type, threshold)
   1092                    error_modeler(param_type = threshold_type,
-> 1093                                  param = threshold)
   1094         else:
   1095            error_modeler(param_type=kwargs['param_type'],

/usr/local/lib/python3.6/site-packages/triage/component/postmodeling/contrast/model_evaluator.py in _error_modeler(self, depth, view_plots, **kwargs)
   1041        zip_data = self._error_labeler(param_type = kwargs['param_type'],
   1042                                   param = kwargs['param'],
-> 1043                                   path=kwargs['path'])
   1044
   1045        # Model tree and output tree plot

/usr/local/lib/python3.6/site-packages/triage/component/postmodeling/contrast/model_evaluator.py in _error_labeler(self, path, param, param_type)
    950         '''
    951
--> 952         test_matrix = self.preds_matrix(path)
    953
    954         if param_type == 'rank_abs':

/usr/local/lib/python3.6/site-packages/triage/component/postmodeling/contrast/model_evaluator.py in preds_matrix(self, path)
    302         storage = ProjectStorage(path)
    303         matrix_storage = MatrixStorageEngine(storage).get_store(self.pred_matrix_uuid)
--> 304         mat = matrix_storage.design_matrix
    305
    306         # Merge with predictions table and return complete matrix

/usr/local/lib/python3.6/site-packages/triage/component/catwalk/storage.py in design_matrix(self)
    392     def design_matrix(self):
    393         """The matrix without the label vector, only the index and features"""
--> 394         return self.matrix_label_tuple[0]
    395
    396     @property

/usr/local/lib/python3.6/site-packages/triage/component/catwalk/storage.py in matrix_label_tuple(self)
    380         if self._matrix_label_tuple:
    381             return self._matrix_label_tuple
--> 382         design_matrix, labels = self._preprocess_and_split_matrix(self._load())
    383         if self.should_cache:
    384             self._matrix_label_tuple = design_matrix, labels

/usr/local/lib/python3.6/site-packages/triage/component/catwalk/storage.py in _load(self)
    624
    625     def _load(self):
--> 626         with self.matrix_base_store.open("rb") as fd:
    627             return pd.read_csv(fd, compression="gzip", parse_dates=["as_of_date"])
    628

/usr/local/lib/python3.6/site-packages/triage/component/catwalk/storage.py in open(self, *args, **kwargs)
    151
    152     def open(self, *args, **kwargs):
--> 153         return open(self.path, *args, **kwargs)
    154
    155

FileNotFoundError: [Errno 2] No such file or directory: '/triage/matrices/7c26b34566316f866687cb5c821f5064.csv.gz'
#+END_EXAMPLE
:END:

* WORKING Crosstabs: How are the entities classified?
...
models_list_query: "select unnest(ARRAY[208, 210]) :: int as model_id"
...
features_query: |
  ...
  from features.inspection_types_aggregation_imputed as f1 join
