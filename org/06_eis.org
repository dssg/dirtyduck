...
with significant_features as (select feature_column, threshold_unit from test_results.eis_crosstabs where metric = 'ttest_p' and value < 0.05 and as_of_date = '2018-09-01') select * from test_results.eis_crosstabs where metric = 'ratio_predicted_positive_over_predicted_negative' and feature_column in (select feature_column from significant_features);
