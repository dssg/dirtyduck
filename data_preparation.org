...
** Downloading
   ...
   We can check the size of the data set and the total number of rows:
...
** Loading data to the database
   Assuming that you are already inside =bastion= (the docker container), run the following:
   ...
   (If you are connected to the database, you can just type =select count(*) from raw.inspections=)
   ...
   This is different from the number of rows that we calculated
   ...
   For more details about the data, check the documentation (located  [[https://data.cityofchicago.org/api/assets/BAD5301B-681A-4202-9D25-51B2CAE672FF?download=true][here]]),
...
If everything works, there should be two new schemas: =cleaned= and =semantic=.
...
** Exploring the data
   ...
   *entities* of your problem domain, and if your problem involves time,
   ...
   We will implement that concept in two different tables, one for entities and
   one for events (named =entities= and =events= respectively).

In this problem, the entity is the *facility*, and the events that happen to that entity
are the *inspections*.
...
Why? Because if you make some mistake, or if you want to
try a different thing you will always can go back to the beginning and
start over.


  In this dataset, the meaning of some columns is the following:
...
Ok, there are two more than we expect: =All= and =NULL= for a grand total of *5* types of risk. What about =types= of inspections?
...
Wow, we got *108* types of inspections instead of the expected *5*.
What are those types? How bad is this issue?
...
So, it seems like this columns will require also cleaning. Finally, let's look =results=
...
Ok, this is also not what we expected. This is disheartening. But, that is the reality of /real/ data. Now, let's try to clean this mess.

** Transforming (and cleaning) the data

Let's start by figuring out how to transform the data. We
will concentrate at first in all columns except =violations=, we will
deal with that later, since is more complex.

First, we will remove superfluous spaces and convert the columns
=type,results, dba_name, aka_name, facility_type, address, city= to
lower case. 

For the column =risk= , we will keep only the description (e.g. *high*).
We will impute as =high= all the =NULL= and =All=
values.
...
As we have seen (and will continue see), /data is always messy/. To
...
All the rows in the data should be from Ilinois so let's check that first:
...
There are 144 rows with =NULL= values and 41 rows with =CCHICAGO= which is clearly a misspelling, the
rest are from different places, but all of them are near Chicago. We will
fix this problem by replacing them with the value =chicago=.

The zip code field has a similar problem:
...
We will remove the column =sate=, =city= since almost everything happens in Chicago (this is
the Chicago's food inspection data set anyway).

We will convert the coordinates latitude and longitude to a =Point=.
We will drop the columns =latitude= and =longitude=, since these are (now) redundant.
...
Finally, we should create from the cleaned tables the =entities= and =events= table.
...
This looks weird, let's investigate a little more about =license_num= = =0=.
...
There is too much uncertainty in this field - we conclude that we can't use the =license_num= as the unique
identifier.
...
Let's use the combination of =license_num=, =facility=, =facility_aka=,
...
all the attributes that describe the entity and doesn't depend on any
...
don't want to make predictions on entities that aren't active. We will
use the following definition

#+BEGIN_QUOTE
A facility is active from the time that they started as a business until it is closed down.
#+END_QUOTE
...
with entities_date as (
  ...
  license_num, facility, facility_aka, facility_type,
...
We are now ready to create our events table. This table describes the data related to the inspections. 
We also add columns from the facility inspected that will simplify the creation of /features/ for our
