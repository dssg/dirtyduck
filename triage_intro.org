...
not active). For testing purposes this table only include one facility.
...
from
(select inspection, entity_id, zip_code, jsonb_array_elements(violations::jsonb) as obj from semantic.events)
as t1
...
#+BEGIN_EXAMPLE sql
...
SELECT entity_id, '2014-10-08'::date AS date,
sum(critical_violations) FILTER (WHERE date >= '2014-10-08'::date - interval '1y') AS violations_entity_id_1y_critical_sum,
avg(critical_violations) FILTER (WHERE date >= '2014-10-08'::date - interval '1y') AS violations_entity_id_1y_critical_avg,
stddev(critical_violations) FILTER (WHERE date >= '2014-10-08'::date - interval '1y') AS violations_entity_id_1y_critical_stddev,
sum(serious_violations) FILTER (WHERE date >= '2014-10-08'::date - interval '1y') AS violations_entity_id_1y_serious_sum,
avg(serious_violations) FILTER (WHERE date >= '2014-10-08'::date - interval '1y') AS violations_entity_id_1y_serious_avg,
stddev(serious_violations) FILTER (WHERE date >= '2014-10-08'::date - interval '1y') AS violations_entity_id_1y_serious_stddev,
sum(low_violations) FILTER (WHERE date >= '2014-10-08'::date - interval '1y') AS violations_entity_id_1y_low_sum,
avg(low_violations) FILTER (WHERE date >= '2014-10-08'::date - interval '1y') AS violations_entity_id_1y_low_avg,
stddev(low_violations) FILTER (WHERE date >= '2014-10-08'::date - interval '1y') AS violations_entity_id_1y_low_stddev
FROM
(
select inspection, entity_id, zip_code, date,
count(*) filter (where obj ->> 'severity' = 'critical') as critical_violations,
...
WHERE date < '2014-10-08'AND date >= '2014-10-08'::date - greatest(interval '1y') GROUP BY entity_id
...
#+END_EXAMPLE
...
facilities_nearby as (
   select distinct a.entity_id, a.location, a.facility_type, b.facility_type as other_facility_type
...
group by
entity_id, location, facility_type, other_facility_type;
#+END_SRC

#+RESULTS:
:RESULTS:
| entity_id | location                                           | facility_type | other_facility_type                | count |
|------------+----------------------------------------------------+--------------+----------------------------------+-------|
