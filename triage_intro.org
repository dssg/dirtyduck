...
Triage provides interfaces to these different phases of a
...
In this tutorial, we have two different goals: (1) *EIS* (Early Information System) and
   ...
   next time period. TODO: add link
   ...
   violation /given that/ they are inspected. TODO: add link


** The problem to solve

Both these problems have a temporal nature to the data. As a result,
the most acute problems are avoiding leakage (from the future to the past) 
and feature generation in a temporal setting

** Temporal crossvalidation

We configure the parameters of our temporal experiment set up in ...yaml.
This includes information about the time period over which the labels and features are computed, the frequency of model retraining, etc. [see section appendix for more details]

It is crucial to specify this correctly to avoid leakage of information where
you are training on information from future time points. (note: better way to describe?)
...
* Appendix: Temporal Configuration Parameters
