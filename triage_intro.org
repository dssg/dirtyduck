...
- =test_durations= :: how far into the future should a model be used
     to make predictions *NOTE*: in the typical case of wanting a single
     prediction set immediately after model training, this should be
     set to 0 days  (test span)
...
as dashed vertical black lines. This setup would be our *base* example.
...
#+NAME: fig:timechop_3
#+CAPTION: A smaller model_update_frequency (from 1y to 6month)
#+ATTR_ORG :width 100 :height 100
#+ATTR_HTML :width 600 :height 400
#+ATTR_LATEX :width 400 :height 300
