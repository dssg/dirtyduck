...
#+BEGIN_SRC sql
  select distinct risk from raw.inspections;
#+END_SRC
...
#+BEGIN_SRC sql
select type, count(*) from raw.inspections group by type order by count(*) desc limit 15
#+END_SRC
...
#+BEGIN_SRC  sql
  select distinct results from raw.inspections
#+END_SRC
