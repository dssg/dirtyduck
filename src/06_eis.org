...
=RandomForestClassifier=, 1 =DummyClassifier=) \times *9* features groups (1
=all=, 4 =leave-one-in=, 4 =leave-one-out=). The total number of /models/
is triple that (we have 3 time blocks, so *96*).
...
#+BEGIN_SRC sh :dir /docker:root@tutorial_bastion:/ :results silent
# Remember to run this in bastion  NOT in your laptop shell!
triage experiment experiment_config/inspections_dt.yaml --profile
#+END_SRC
