...
In this experiment we will end with *20* model groups
(number of algorithms [1] $\times$ number of hyperparameter combinations [2
$\times$ 2 = 4] $\times$ number of feature groups strategies [1 $\plus$ 4 =
5]). Also, we will create *60* models (3 per
...
create *30* matrices (3 for
training, 3 for testing, 5 per each feature group strategy) in =triage/matrices= and every matrix will be
